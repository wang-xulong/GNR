...
class GetLr(SupervisedPlugin):
    def before_training_epoch(self, strategy, **kwargs):
        print("Current learning rate:", strategy.optimizer.state_dict()['param_groups'][0]['lr'])


class PrintModel(SupervisedPlugin):
    def before_eval(self, strategy, **kwargs):
        print(strategy.model)

class MyEarlyStoppingPlugin(SupervisedPlugin):
    ...
    def _update_best(self, strategy):
        ...
        current_task = f"Task{strategy.experience.current_experience:03d}"
        names = [k for k in res.keys() if k.startswith(self.metric_key) and current_task in k]
        if len(names) == 0:
            return None
        if not names:  # 如果没有匹配的指标，返回 None
            warnings.warn(f"No matching metric found for task {current_task}.")
            return None
...
def compute_gradient_norm(model):
    ...
    for param in model.parameters():
        if param.grad is not None:  # 确保参数有梯度
            param_norm = param.grad.data.norm(2)  # 计算梯度的L2范数
            total_norm += param_norm.item() ** 2
    total_norm = total_norm ** 0.5  # 总范数的平方根
    return total_norm
...
class GradientNormPlugin(SupervisedPlugin):
    def __init__(self):
        super().__init__()
        self.after_eval_exp_gradient_norms = []
        self.after_training_epoch_gradient_norms = []

    # def after_eval_exp(self, strategy, **kwargs):
    #     grad_norm = compute_gradient_norm(strategy.model)  # 计算梯度模长
    #     self.after_eval_exp_gradient_norms.append(grad_norm)
    #
    #     print(f"experience {strategy.clock.total_iterations}, after_eval_exp Gradient Norm: {grad_norm}")

    def after_training_epoch(self, strategy, **kwargs):
        grad_norm = compute_gradient_norm(strategy.model)  # 计算梯度模长
        self.after_training_epoch_gradient_norms.append(grad_norm)
        print(f"train_exp_epochs {strategy.clock.train_exp_epochs}, after_training_epoch Gradient Norm: {grad_norm}")
